...
http_request POST "$BASE_URL/api/sub-tasks/$SUB_TASK_ID/start" "" "X-Admin-Token: $ADMIN_TOKEN"
...
http_request POST "$BASE_URL/api/sub-tasks/$SUB_TASK_ID/submit" "" "X-Admin-Token: $ADMIN_TOKEN"
assert_eq "200" "$HTTP_CODE" "O submit 子任务失败"
log "submit Body: $HTTP_BODY"
log ""

log "=== [P] REST complete subTask -> DONE ==="
http_request POST "$BASE_URL/api/sub-tasks/$SUB_TASK_ID/complete" "" "X-Admin-Token: $ADMIN_TOKEN"
