    ...
    <el-card>
      <template #header>
        <div class="card-header">
          <span>收件箱</span>
          <div>
            <el-tag v-if="unreadCount > 0" type="danger" size="small">未读 {{ unreadCount }}</el-tag>
            <el-button size="small" @click="load" class="ml-sm">刷新</el-button>
          </div>
        </div>
      </template>
      <el-table :data="list" border stripe v-loading="loading" style="width:100%">
        <el-table-column label="类型" width="120">
          <template #default="{ row }">
            <el-tag size="small" :type="eventTypeTag(row.eventType)">{{ INBOX_EVENT_TYPE_MAP[row.eventType] || row.eventType }}</el-tag>
          </template>
        </el-table-column>
        <el-table-column prop="title" label="标题" min-width="240" show-overflow-tooltip />
        <el-table-column label="优先级" width="80">
          <template #default="{ row }">
            <el-tag v-if="row.priority==='URGENT'" size="small" type="danger">紧急</el-tag>
        ...
        <el-table-column label="操作" :width="ACTION.TWO" fixed="right">
          <template #default="{ row }">
            <div class="action-cell">
              <el-button v-if="row.isRead===0" size="small" type="primary" @click="markRead(row)">标为已读</el-button>
              <el-button size="small" @click="markArchived(row)">归档</el-button>
            </div>
          </template>
        </el-table-column>
      </el-table>
      <el-empty v-if="!list.length && !loading" description="收件箱为空" />
    </el-card>
  </div>
</template>
