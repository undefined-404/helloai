...
function eventTypeColor(eventType: string): '' | 'success' | 'warning' | 'danger' | 'info' | 'primary' {
  ...
  if (eventType.includes('paused') || eventType.includes('warning') || eventType.includes('unparseable') || eventType.includes('degraded') || eventType.includes('disagreement') || eventType.includes('discrepancy') || eventType.includes('incomplete')) return 'warning'
...
function eventCategory(eventType: string): EventCategory {
  ...
  if (/dispatch|command|assigned/.test(eventType)) return '分发'
