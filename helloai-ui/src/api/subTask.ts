...
export const subTaskApi = {
  ...
  list(params: { taskId?: LongId; status?: string; page: number; pageSize?: number }) {
    return request.get<any, PageResult<SubTask>>('/sub-tasks/list', { params })
  },
  // 按主任务拉全量子任务（不传 page 走后端全量数组契约；依赖图与序号映射用）
  listAllByTask(taskId: LongId) {
    return request.get<any, SubTask[]>('/sub-tasks/list', { params: { taskId } })
  },
  // v1.1 修复: LongID 后端已全局序列化为 string，传 string 避免任何 Number() 精度丢
  getById(id: LongId) {
    return request.get<any, SubTask>(`/sub-tasks/getById/${id}`)
  },
  ...
  timeline(id: LongId) {
    return request.get<any, TaskTimelineItem[]>(`/sub-tasks/listTimelineBySubTaskId/${id}`)
  },
  // V28: 子任务执行对话流（执行产出全文 + 核验 Prompt/分析原文）
  conversation(id: LongId) {
    return request.get<any, ConversationMessageItem[]>(`/sub-tasks/listConversationBySubTaskId/${id}`)
  },
  // V25: 死信人工兜底指派（DEAD_LETTER → ASSIGNED，熔断计数清零）
  redispatchDeadLetter(id: LongId, agentId: LongId) {
    return request.post(`/sub-tasks/redispatchDeadLetterById/${id}`, { agentId })
  },
  // BLOCKED 子任务重新调度（reset → PENDING 后交调度链，dispatchBlockedSubTask）
  reassign(id: LongId, agentId: LongId) {
    return request.post(`/sub-tasks/reassignById/${id}`, { agentId })
  },
  changeStatus(data: ChangeStatusRequest) {
    return request.post('/sub-tasks/changeStatus', data)
  },
  claim(id: LongId, agentId: LongId) {
    return request.post(`/sub-tasks/claimById/${id}`, null, { params: { agentId } })
  },
  start(id: LongId) {
    return request.post(`/sub-tasks/startById/${id}`)
  },
  submit(id: LongId) {
    return request.post(`/sub-tasks/submitById/${id}`)
  },
  block(id: LongId) {
    return request.post(`/sub-tasks/blockById/${id}`)
  },
  mine(agentId: LongId) {
    return request.get<any, SubTask[]>('/sub-tasks/listMine', { params: { agentId } })
  },
  available() {
    return request.get<any, SubTask[]>('/sub-tasks/listAvailable')
  },
  pause(id: LongId) {
    return request.post(`/sub-tasks/pauseById/${id}`)
  },
  resume(id: LongId) {
    return request.post(`/sub-tasks/resumeById/${id}`)
  }
}
