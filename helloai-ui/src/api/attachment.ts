...
export const attachmentApi = {
  list(subTaskId?: LongId) {
    return request.get<any, Attachment[]>(paths.attachments.list, { params: { subTaskId } })
  },
  getById(id: LongId) {
    return request.get<any, Attachment>(paths.attachments.getById(id))
  },
  // 下载附件：local:// 物化产物由后端流式返回；blob 响应在拦截器放行，
  // 返回完整 response 供 saveBlobResponse 解析文件名
  download(id: LongId) {
    return request.get<any, AxiosResponse<Blob>>(paths.attachments.download(id), { responseType: 'blob' })
  },
  ...
  previewById(id: LongId) {
    return request.get<any, AxiosResponse<Blob>>(paths.attachments.preview(id), { responseType: 'blob' })
  },
  ...
  previewTextById(id: LongId) {
    return request.get<any, AxiosResponse<string>>(paths.attachments.preview(id), { responseType: 'text' })
  }
}
