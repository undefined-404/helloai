...
export const clarifyApi = {
  ...
  send(id: LongId, message: string, selectedOptions?: ClarifySelection[] | null) {
    return request.post<any, ClarifyConversationDetail>(
      `/requirement-conversations/${String(id)}/messages`,
  ...
  retry(id: LongId) {
    return request.post<any, ClarifyConversationDetail>(
      `/requirement-conversations/${String(id)}/retry`, null, { timeout: 120_000 })
  },
  ...
  // 会话详情（含全部消息按 seq 升序）
  detail(id: LongId) {
    return request.get<any, ClarifyConversationDetail>(`/requirement-conversations/${String(id)}`)
  },
  // 终稿确认: 创建任务（PENDING）并回填会话 FINALIZED
  finalize(id: LongId) {
    return request.post<any, Task>(`/requirement-conversations/${String(id)}/finalize`)
  },
  // 重新生成: FINALIZED 会话原任务已删除时，复用终稿重建任务
  regenerate(id: LongId) {
    return request.post<any, Task>(`/requirement-conversations/${String(id)}/regenerate`)
  },
  // 放弃会话: ACTIVE → ABANDONED
  abandon(id: LongId) {
    return request.post<any, void>(`/requirement-conversations/${String(id)}/abandon`)
  }
}
