...
> 定位说明
>
> - 本文件保留为路线图/目标态基线，用于说明 HelloAI 中长期规划、阶段拆分与设计意图。
> - 本文件不再作为“当前仓库已全部实现”的判断依据，也不再持续堆叠逐轮执行日志。
> - 当前项目现实边界请看 `doc/HelloAI_项目基线文档.md`。
> - 当前实现与目标差异请看 `doc/HelloAI_实现差距表.md`。
> - 每轮实施结果请看 `doc/HelloAI_迭代执行记录.md`。
> - 版本说明：本文件中关于 `spring-ai 1.1.0` 的锁定结论属于当时阶段性决策；当前仓库运行版本与最新回归结果请以 README、项目基线文档和迭代执行记录为准。
...
### F.8 spring-ai 1.1.0 永久稳定版锁定（历史决策归档，已被后续验证结论覆盖）

> 归档说明
>
> - 本节保留 2026-07-09 当时的版本决策背景，供后续追溯使用。
> - 当前仓库运行版本已为 `spring-ai 1.1.8`。
> - `spring-ai 1.1.8` 已在 macOS 下通过 `verify-mcp-auth.sh` + `verify-mcp-e2e.sh` 主链路回归。
> - 因此，本节不再作为现行生效规则；现行口径请以 README、`doc/HelloAI_项目基线文档.md` 与 `doc/HelloAI_迭代执行记录.md` 为准。
