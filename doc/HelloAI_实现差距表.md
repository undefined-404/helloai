...
| N6 | 执行命令消费与结果回写 | 主链路已连通（producer/consumer 独立开关，E2E 待验证） | 已完成 DB Poller 主线化：`consumer-mode` 支持 `EVENT / POLLER / BOTH`，默认 `POLLER`；`ExecutionCommandService` 在 `EVENT/BOTH` 才发布本地事务事件，`POLLER` 仅落库 PENDING 命令；`ExecutionCommandPoller` 在 `POLLER/BOTH` 扫描全部 PENDING（主消费），在 `EVENT` 仅扫描孤儿 PENDING（兜底）；Poller 依赖抽象 `ExecutionCommandConsumer`，默认实现为 `LocalExecutionCommandConsumer.consume()`（Bean 常驻）。结果回写入口已收口为 `ExecutionResultHandler.handleReport(ExecutionResultReport)`，平台内执行链与外部 MCP `submitResult` 统一走该入口。**本轮（Phase 2D）新增 MQ Consumer 骨架：`MqExecutionCommandConsumer` 与 `LocalExecutionCommandConsumer` 共用 `ExecutionCommandConsumer` 接口，`@RabbitListener` MANUAL ACK，`@ConditionalOnProperty(helloai.mq.execution-command.enabled=true)` 默认不注册 Bean；`RabbitMQConfig` 加 `EXECUTION_COMMAND_QUEUE` / `EXECUTION_COMMAND_EXCHANGE` / `executionCommandBinding`（路由 `execution.command.*`）+ 复用 `DLX_EXCHANGE` / `DLX_QUEUE`；8 个单测覆盖 HappyPath / EdgeCases / Deduplication / ChannelIo。**Phase 2E 完成生产端接入：** 新增 `AgentExecutionProperties.DispatchMode`（`NONE / EVENT / MQ / BOTH`，默认 `NONE` 保持零行为变化）+ `ExecutionCommandMqPublisher`（`@ConditionalOnProperty(helloai.mq.execution-command.producer-enabled=true)`，PERSISTENT 投递 + messageId/correlationId=eventId）+ `ExecutionCommandService.publish()` 按 dispatch-mode 分发（EVENT/BOTH 发本地事件；MQ/BOTH 发 MQ；MQ 段 Publisher Bean 不可用直接抛 `IllegalStateException`，**不隐式回退**）+ `ExecutionDispatchValidator`（`@PostConstruct` 启动期 fail-fast 校验 + 配置打印）；`MqExecutionCommandProperties` 拆分为 `producer-enabled` / `consumer-enabled` 双开关（支持独立灰度：先开 producer 观察 shadow 队列堆积，再开 consumer 抵消双消费）；`MqExecutionCommandConsumer` 切到 `consumer-enabled` 且注入 Properties。新增 `ExecutionCommandServiceDispatchTest` 6 用例覆盖 4 模式分发 + 2 类 fail-fast，14 用例全绿。E2E 待 RabbitMQ 环境 ready 后跑 | E2E 待验证：在具备 RabbitMQ 的环境设置 `helloai.execution.dispatch-mode=BOTH` + `helloai.mq.execution-command.producer-enabled=true` + `helloai.mq.execution-command.consumer-enabled=true`，观察幂等抵消双消费；后续切除 Poller 兜底与消费侧回写链路改造留待独立迭代 |
