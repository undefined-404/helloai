> **状态：HISTORICAL / ARCHIVED**。当前开发不得把本文档作为架构依据。
...
- 原 README 未突出平台与单机/容器化多 Agent 框架（CrewAI / LangGraph / AgentTeams）的本质差异——分布式终端调度：聚合 N 台终端的碎片化算力，跨终端、跨产品（Qoder / Trae / Codex CLI / Claude Code）协作，算力上限由终端数量决定而非单台服务器。

#### 2. 实际落地

- 定位重构（纯文档，零代码变更）：标题改为「分布式跨终端 AI Agent 调度平台」；开篇平台定义（跨终端聚合算力 + 拆解/派发/回收/验收四职责）提为首段，「AI 项目经理」使用叙事降为第二段；删除旧「一句话看懂」PNG 引用（与新定位不符，图片文件保留未删）。
- 新增「💎 核心差异化：分布式终端调度」：算力形态对比表（算力上限 / Agent 生态 / 扩容方式 / 协作方式 / 跨产品协作）+ 诚实说明（Agent 侧算力已分布式；调度核心单实例，Redisson + ShedLock 双锁已为多实例扩容打底）。
- 新增「🔬 运行机制（工程视角）」：四张流程图入库 `doc/diagrams/`（子任务端到端流水线 / Agent 选人 12 层硬过滤 + 4 级软排序 / 子任务 11 态状态机 / Reviewer 审核全流程，各含 SVG + HTML 可缩放版），另增开篇架构图 `helloai-architecture.svg`（云端编排层 + 多终端执行层 + 差异化注记卡，图文口径「任务可跨终端，跨厂商：同一主任务的子任务由不同终端的不同 AI 接力完成」）。
- 同步修正：⚖️ 对比表新增「算力形态」行；核心功能 5 与任务生命周期补状态机/审核图内链；路线图待办「分布式调度扩展」口径改为「调度核心多实例水平扩容（Agent 侧算力本身已跨终端分布式）」；文档导航补 `doc/diagrams/`。
- 「跨终端」关键词密度强化（同日追加，用户反馈）：开篇第一段加入指定句「任何能跑 CLI Agent 的笔记本、台式机等终端设备，都是 HelloAI 的算力节点」；正文 6 处自然融入（开篇第二段 / 任务旅程步骤 4 / 运行机制导读 / 弹性调度外部 CLI Agent / 核心概念 CLI_CLIENT / 快速接入步骤 2--末处顺带点明「该终端即成为平台的算力节点」），全文「跨终端」4 处 -> 9 处，均加在语义自然位置不堆砌。

#### 3. 遗留 / 不做的事

- `README.en.md` 未同步（英文版仍为旧结构，待后续单独处理）。
...
### 6.164 内部 LLM Agent 执行凭证平台级化：系统管理轮换 API Key 后旧注册 Agent 实时生效（2026-08-28）

#### 1. 背景与结论

用户报告缺陷：系统管理（模型配置）更换 API Key 后，已注册的内部 LLM Agent（API_KEY_LLM）执行任务报 error，仍用旧 Key，只能删除 Agent 重建（连带删除历史处理信息）。根因：注册时 `LlmProviderCatalogServiceImpl.provisionPlatformCredential → bindPlatformApiKeyIfAbsent` 把平台级密钥**复制**成 AGENT 级快照存入 credential_vault，而执行链 `ApiKeyAgentExecutor` 读取的是 AGENT 级快照；系统管理轮换（`rotatePlatformApiKey`）只动 PLATFORM 级凭证，旧快照永久残留。结论：内部 LLM Agent 不存 API Key 快照，执行密钥按 modelType 关联平台级（llm_provider / credential_vault PLATFORM 级）凭证实时解析——**平台级优先、Agent 级兜底**，轮换后已注册 Agent 实时生效，无需删除重建。明确不做：删除存量 AGENT 级快照（平台级优先后自动失效）、改动 `CredentialController.bindApiKeyByAgentId` 手工绑定链路（脚本自定义 key 场景保留）。

#### 2. 实际落地

- **新增 `AgentLlmCredentialResolver`**（helloai-core/agent/，`@Component` 构造注入）：`resolveApiKey(agent)`——按 `AgentProviderResolver.resolveProvider`（modelType 前缀，缺省 `helloai.execution.provider`）解析 provider，平台级 `platformProviderConfigService.getApiKey(provider)` 非空白即返回，否则回退 `credentialVaultBindingService.getAgentApiKeyPlaintext(agentId, provider)`，双无返回 null；`hasUsableCredential(agent)`——非 API_KEY_LLM 放行，平台级已配置 或 Agent 级 `hasActiveAgentCredential` 即可用，查询异常防御式降级 false（选中无凭证 Agent 必败，排除更安全）。
- **执行链**：`ApiKeyAgentExecutor.execute` 非 mock 分支改 `agentLlmCredentialResolver.resolveApiKey(agent)`（替代 `getAgentApiKeyPlaintext` 直读），错误文案更新为「未配置可用的平台级或 Agent 级 API Key ... 请先在系统管理中配置模型 API Key」。
- **探活链**：`AgentExecutionConnectivityServiceImpl.probe` 同改 resolver 取 key（`hasActiveVaultCredential` 展示口径保留）。
- **选人链**：`AgentSelector.hasUsableCredential` / `PlannerAgentPicker.hasUsableCredential` 委托 resolver，并本地补 try-catch 防御降级（mock/实现异常不穿透选人链），两处移除 CredentialVaultService 依赖。
- **注册链**：`LlmProviderCatalogService` 接口删 `bindPlatformApiKeyIfAbsent`；`LlmProviderCatalogServiceImpl.provisionPlatformCredential` 收敛为平台凭证核对语义（仅校验 provider 可用性，不再向 credential_vault 写 AGENT 级快照）；`AgentController.applyRegistrationExtras` 注释同步。
- **测试同步**：`AgentSelectorTest` / `PlannerAgentPickerTest` mock 与构造器替换（credentialVaultService → resolver，CLI 用例 verify 语义更新）；`LlmProviderCatalogServiceTest` 删除 BindPlatformApiKey 四用例、provisionPlatformCredential 重写为核对语义（4 例）；**新增 `AgentLlmCredentialResolverTest` 11 例**（resolveApiKey 六分支：null/平台优先/前缀解析/平台缺失回退/平台空白回退/双无 null；hasUsableCredential 五分支：null 放行/CLI 放行/平台可用/Agent 可用/双无不可用/异常降级）；`ApiKeyAgentExecutorTest` / `PlatformAgentExecutionServiceTest` 构造器同步（mock 模式不触达 resolver）。

#### 3. 验证结果

- `mvn -pl helloai-core test -DskipTests=false -Dtest=AgentSelectorTest,PlannerAgentPickerTest,LlmProviderCatalogServiceTest,AgentLlmCredentialResolverTest,ApiKeyAgentExecutorTest,PlatformAgentExecutionServiceTest`：**89/89 全绿**（首轮 4 错全修复：resolver 测试 3 处 UnnecessaryStubbing → setUp stub 改 lenient；AgentSelectorTest 凭证异常用例 → 委托处补本地防御）；项目默认 `skipTests=true`，需显式 `-DskipTests=false`。
- `mvn -pl helloai-core -am compile` 及全模块 `mvn compile`：BUILD SUCCESS 零错误；全仓库 `bindPlatformApiKeyIfAbsent` 代码引用零残留（仅文档历史记录保留）。
