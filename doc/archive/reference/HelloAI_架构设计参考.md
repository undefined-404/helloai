...
> **状态注记（2026-08-23 文档核查）**：本文档为设计理念/目标态方向参考，正文保留 2026-07-11 拍板原貌。其中几处结论已被后续交付推进/推翻，阅读时请注意：
> - §5.0「Planner 用 LLM 自动拆解子任务 = 暂缓」：已于 2026-07-28 按用户决策**提前启动并交付**（V26 平台内自动拆解，见差距表 N16）；
> - §5.0「唯一缺失的硬骨头 = 长连接门铃通知」：门铃 SSE 通道（AgentHub V3，N13）已交付，但 2026-08-07 因外部 Agent 为单向执行器、无法消费平台推送而**搁置**（代码保留运行，待 Agent 端常驻 daemon 落地后可复用）；
> - §5.0 门铃 + Bridge 路线（2026-09-05 复核）：外部 Agent 实时协作闭环的价值假设只在「无人值守」场景成立，而各 CLI 无人值守 headless 执行不可行、会话内 SKILL 模式已覆盖有人值守场景——**V2 轻量 Bridge 砍除**（A1 方案 R2），门铃服务端保留为已运行资产；P2-A 外部协作主线替换为「接单类 SKILL 试点」（LOG-20260905-004）；
> - §5.1「MQ 主链路线拍板」②a/②b/③：均已交付（Outbox 一体化 + Confirm/Retry + Poller 兜底重塑，见差距表 N1/N6）。
> 本文档不承担实现状态职责，当前事实以《实现差距表》与《迭代执行记录》为准。
