...
> **状态注记（2026-08-03 文档核查）**：本文档正文中的"现状"描述（§3 主链路图/职责表、§4 差异表）反映的是 2026-07-10 的代码快照，仅供参考历史拆解思路。其目标态（调度只发命令 / 执行独立消费 / 结果异步回写 / 最终一致收敛）已由后续迭代落地：`ExecutionCommand` + MQ 主链 + Outbox 可靠投递 + Poller 兜底（差距表 N1/N6）、值班优先调度（N12）、门铃通知（N13）等。当前执行链事实以《实现差距表》《项目基线文档》与《迭代执行记录》为准，本文档不作为现状说明。

> **状态注记（2026-08-23 文档核查）**：§5 目标结构四层（调度层 / 执行消费层 / 结果处理层 / 收敛层）已全部落地：`ExecutionCommandConsumer` 接口 + `LocalExecutionCommandConsumer` / `MqExecutionCommandConsumer` 双实现、`ExecutionResultHandler` 固化为唯一执行结果入口、Reconcile / 超时补偿任务收口兜底（差距表 N1/N6）。
