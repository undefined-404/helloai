...
- **回填**：差距表 N-010 按完成规则移除（总览行 + §13 章节，后续重排编号，结论清单去「MQ / 异步链路进一步治理」，最后更新 2026-09-05）；A3 方案矩阵 A4 备注同步修订（R3）；LOG-20260905-010。C1 §10 四决策点审阅
⚠️ 先报一个影响决策 1/4 的事实问题
C1 D2/§3.4 声称节点约束实例化时「渲染为 sub_task.title/deliverable/acceptance/context/required_skills 与 task.agent_policy 节点级覆盖」——经 V1 核对：
sub_task 表没有 required_skills 列（该列是 task 级 V47 加的）；sub_task 实有：title/status/content/context/deliverable/acceptance/priority/deadline/score* 等；
task.agent_policy 是任务级单值（JSONB 存一组 executorAgentIds 等），"节点级覆盖"在现模型下不可表达——一个 task 的所有 sub_task 共享同一 agent_policy。
推论：节点级"让不同节点用不同执行者"的能力，现状为零（要么全任务同组合，要么手工指派 assigned_agent_id）。这不是 bug，是 N-002 Team 的真实缺口，也是决策 4 切分线的最佳锚点（见下）。
决策 1：spec 字段落 context（倾向：落 context，零 DDL）→ 支持，且补两条实施约束
项	分析
结论	落 context 成立。事实佐证：sub_task.context 是 JSONB、全部既有写入方（拆解/派发/返工/结果回写）都是增量 putAll patch 语义，无 schema 强约束，追加键零冲突（已核实 SubTaskServiceImpl.changeStatus 的 contextPatch 合并逻辑）
约束 1	别平铺散键。context 键空间无 registry，各写入方自有键（如 llmDraftSource 类）。建议模板来源标记用单一顶级键 "workflow": {nodeKey, spec:{estimatedEffort,scope,...}, templateVersionId} 整体装入，避免 node_key/spec 平铺与未来其他写入方撞名——这条建议并入 §10-1 拍板说明
约束 2	可渲染列要改口径：spec/constraints 实际只能渲染到 V1 实有列（title/deliverable/acceptance/content）+ context 扩展键；required_skills 渲染目标删除，节点级能力约束首版不表达（见决策 4）
决策 2：失败续跑（倾向：首版不支持，人工重建）→ 支持，但要先拆清两级恢复语义
级	形态	现状	判定
节点级恢复	BLOCKED/FAILED 子任务 → 人工改派/rework	已存在且成熟（重派链 + B1 恢复上下文注入 appendRecoveryContext）	与 Workflow 无关，天然可用
实例级续跑	新实例保留已完成节点、只重跑失败节点	需要 Workflow 层感知节点完成度 → 违背 D1"一次性物化、零运行期干预"	首版砍，正确
必须防的反模式：如果 workflow_instance.status=FAILED 被实现成能反锁已物化 task/sub_task 的操作（例如 FAILED 后禁止改派），就违背 D1"实例化后脱离 Workflow 层"——实例状态必须是派生态，绝不反向约束 task。落到设计上建议二选一，在 §4.2 拍板：
A（推荐，最简）：实例状态不落列、纯查询聚合（每次现算节点完成度），天然无一致性与反锁问题——与 D3"状态聚合从 sub_task 推导"完全同构，连 §3.3 的 status 列都可省（列只留 task_id 绑定）；
B：落列但语义声明为"最近聚合快照（展示用，无状态机、无约束力）"，需处理 FAILED→RUNNING 回升规则（人工改派后节点复活，聚合应回升）。
人工重建 = 新建 workflow_instance 实例化到新 task，旧 task 保留为失败现场（审计/复盘），不做旧 task 复活。
决策 3：接一次 LLM 微调（倾向：首版不接）→ 支持，且根因比"意愿"更硬
首版不接的正确性不只在边界控制——现有链路根本没有"已物化 DAG 的 LLM 修订通道"：
现有 Planner 链路形态是「LLM 生成 PlanDraft 草案 → 人工确认 → 物化 sub_task」——LLM 只写草案，物化后改子任务集（增删/重排 depends_on）不属于 Planner 能力；
若"实例化后接微调"，等于要求新造一条「模板物化 → LLM 改已物化 sub_task」通道，这是新能力而非复用；
未来若接，正确接点是"实例化前"：模板+params 作为 planner 的输入（模板约束下的草案生成），走既有草案-确认链路产出变体——这才是复用现有链。C1 §9 风险表"模板实例可后续接 Planner 微调"应精确为此口径；
另：D4 版本管理本身就是"微调"的确定性替代（改模板 + 升版本），LLM 修订的不确定性反而损害模板复用价值。
决策 4：首版边界 + N-002 Team 切分（你要求重点分析）→ 切分主轴：Workflow 管"流程形状"，Team 管"槽位填充"
切分线一句话：Workflow 模板节点只声明 role + spec + 约束，不做 agent 绑定；"哪组 agent 来填这些角色"不在 Workflow 内核里持久化——实例化时以模板默认 agentPolicy（任务级单值）填充，或留空交调度/人工。N-002 Team 的职责 = "命名可复用的 agent 组合"（如某项目组 = plannerA + executorB,C + reviewerD），未来以 params.teamId 形式成为实例化的可选参数源（填充 task.agent_policy）。这样：
Workflow 首版不依赖 Team 也可用（默认策略/留空），Team 上线后只是多一个参数源，不侵入 Workflow 内核——依赖方向单向：Workflow 不引用 Team，Team 可喂 Workflow；
节点级差异化（节点 A 用 X、节点 B 用 Y）被显式划到 Team/后续：它需要"节点级 agent 绑定 + 调度按节点元数据匹配"，现状 task.agent_policy 任务级单值表达不了（见开头事实问题）。首版文档直接声明"模板粒度只到 role，节点不绑定 agent"，把 C1 D2 中"task.agent_policy 节点级覆盖"的不现实表述删掉；
条件分支/循环：depends_on 是静态 DAG、实例化时一次性展开——条件分支需运行期决策，循环可用"参数数组展开 N 组"在静态框架内近似，但首版两者都不做（YAGNI），文档已正确砍除，与 N-002 无交叠；
N-003 Browser（P2）：模板约束将来可带 access_type 提示字段，实例化落入任务级约束，调度匹配不上时由现有 fallback 兜底——模板预留字段不预读，不感知 N-003 是否存在。
拍板后建议的 C1 修订点（4 处文本）
位置	修订
D2 节点模型落点段	"渲染为 …sub_task.required_skills" → 改为 title/deliverable/acceptance/content + context 单顶级键 workflow；删"task.agent_policy 节点级覆盖"表述 → "模板级默认 agentPolicy（任务级单值）"
§3.4	来源标记明确单顶级键 context.workflow = {nodeKey, spec, templateVersionId}
§4.2	实例状态语义拍板：纯查询聚合（推荐）或快照列；补"实例状态永不反锁 task/sub_task 操作"声明
§10	决策 1 并入上述 context 键空间约束；决策 4 结果（节点不绑 agent、模板默认单值）回填 D2/§8 不做项