...
- **后补注记（2026-08-21）**：上条“本轮明确不在范围”中两项已在后续轮次因实际命中问题再评估后落地，不再是永久决策——① 每轮重新检索已自 V41 放宽为 CLARIFY 每轮触发（见 §6.105）并扩展至 CHAT 模式（见 §6.111）；④ 检索词 LLM 改写已自 V45 落地为“规则清洗 + 条件 LLM 改写 + 多候选词顺序降级”（触发条件即本条预设的“遇不命中”：博查零结果真实案例，见 §6.131）。其余 ②③⑤ 项仍维持不实现口径。
